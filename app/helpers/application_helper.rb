module ApplicationHelper
  def sidebar_link_to(name, path, icon: nil)
    ...
    base_classes = "flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors"
    active_classes = is_active ? "bg-gray-100 text-gray-900" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"

    link_to path, class: "#{base_classes} #{active_classes}" do
      concat(sidebar_icon(icon)) if icon.present?
      concat(name)
    end
  end
